...
def GA1_6(question, file_path=None):
    try:
        ...
        if url_match:
            ...
            # If file_path is an UploadFile or a file-like object
            if hasattr(file_path, "file"):
                file_path.file.seek(0)  # Ensure pointer is at the start
                html_data = file_path.file.read().decode("utf-8")
            else:
...
def GA1_11(question, file_path=None):
    try:
        # If file_path is provided, read from file, otherwise use question as HTML data
        if file_path:
            with open(file_path, "r", encoding="utf-8") as file:
                html_data = file.read()
        else:
        ...
        # Convert data-value attributes to float and sum them up
        values = [float(div['data-value']) for div in divs]
        return int(sum(values))
    except Exception as e:
        print(f"Error in GA1_11: {e}")
        return ""
